...
const getPeoples = () => {
  try {
    return axios.get('https://swapi.co/api/people', 'https://swapi.co/api/planets/1/')
  } catch (error) {
    console.error(error)
  }
}
    ...
    .then(response => {
      if (response.data.results) {
        ...
        for (let index = 0; index < x.length; index++) {
          ...
          buat.textContent = element.created;

          const ubah = document.createElement('td');
          ubah.textContent = element.edited;
          ...
          star.appendChild(buat);
          star.appendChild(ubah);
        }
      }
    })
    .catch(error => {
